...
public class RTSearch {
	...
	public static void main(String[] args) throws IOException {
		TrapentrixFinder("matching_rt_generators_m12.txt", 10, 3, 4);
	}
	
    public static void TrapentrixFinder(String filename, int axisDepth, int nCyclesA, int nCyclesB) throws IOException {
        ...
        for (int vertex1 = 1; vertex1 <= 20; vertex1++) {
            ...
            if (vertex1 !=7 && vertex1 != 4) continue;
            ...
            for (int[] a : permutationsA) {
                ...
                for (int[] b : permutationsB) {
                    ...
                    */
                    ...
                    boolean[][] fixedCycleIndices = new boolean[2][];
					

        
                    generator[0] = new int[a.length][];
					for (int i = 0; i < a.length; i++) {
						fixedCycleIndices[0] = new boolean[a.length];
						fixedCycleIndices[0][i] = true;
						generator[0][i] = axisA[a[i]];
					}
                    generator[1] = new int[b.length][];
					for (int i = 0; i < b.length; i++) {
						fixedCycleIndices[1] = new boolean[b.length];
						fixedCycleIndices[1][i] = true;
